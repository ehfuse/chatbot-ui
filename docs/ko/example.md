...
`example/vite.config.ts` 가 두 가지를 갈아끼운다.

- `@ehfuse/chatbot-ui` → `../src` (dist 가 아니라 소스를 봐서 수정이 즉시 반영된다)
- `entity-client` → `src/mocks/entityClient.ts` (앱 서버 없이 메모리에서 API·realtime 을 흉내 낸다)

예제 화면은 소개 / 상담 대화창 / 챗봇 관리 5탭 / 지식 편집 창 네 개이고,
위쪽 **교육자·본사** 스위치로 권한별 화면 차이를 바로 볼 수 있다.
목업 저장소는 메모리라 새로고침하면 처음 상태로 돌아간다.
